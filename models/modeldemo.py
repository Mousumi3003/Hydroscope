...
#%%
area=50990
data = pd.read_csv('3005.csv',index_col='Date',parse_dates=True)
data.index = pd.to_datetime(data.index,format="%d-%m-%Y")
data['SF']=data['SF']*86400*10**3/(area*10**6)
data['PET'] = 0.0053 * (data['T'] + 17.8) * ((data['T'].max()- data['T'].min())**0.5) * 25
...
# --- Top subplot: Time series of Qsim and Qobs ---
axes[0].plot(simulation['Qsim'], label='Simulated Discharge', color='blue', linewidth=2)
axes[0].plot(simulation['Qobs'], label='Observed Discharge', color='orange', linewidth=2)
axes[0].set_title('Time Series: Simulated vs Observed Discharge', fontsize=14)
axes[0].set_ylabel('Discharge (m³/s)', fontsize=12)
axes[0].legend()
axes[0].grid(True, linestyle='--', alpha=0.6)

# --- Bottom subplot: Scatter plot of Qobs vs Qsim ---
axes[1].scatter(simulation['Qsim'], simulation['Qobs'], color='green', alpha=0.7, edgecolor='k')
axes[1].set_title('Scatter Plot: Observed vs Simulated Discharge', fontsize=14)
axes[1].set_xlabel('Simulated Discharge (mm/day)', fontsize=12)
axes[1].set_ylabel('Observed Discharge (mm/day)', fontsize=12)
