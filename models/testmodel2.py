...
def model(
    ...
    """
    Run the MGH hydrological model (variant 2).

    Parameters:
    data (DataFrame): Input data containing 'PPT', 'PET', and 'SF'.
    All other parameters are keyword arguments with default values.
    ...
    for i in range(length):
        # Storage capacity
        storage_expr = (Srz_arr[i] / Srzmax) ** b + c
        if storage_expr > 1:
            S[i] = 1
        ...
        # Evapotranspiration (linear form)
